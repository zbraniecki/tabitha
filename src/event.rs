...
impl Event {
    ...
    /// Check if this is a key repeat event (auto-repeat from holding a key).
    ///
    /// Key repeat events are generated by the OS when holding a key down.
    /// Returns false for non-key events or if the key event kind is not available.
    #[inline]
    pub fn is_key_repeat(&self) -> bool {
        match self {
            Event::Key(key) => key.is_repeat(),
            _ => false,
        }
    }
}
